...
# --- Script Description ---
#
SCRIPT_VERSION = "v25.04"
SCRIPT_DATE    = "06-July-2025"
#
# A. Gehri/Gemini
#
# This script compares two folders (source and target) to identify and report on
# files based on their existence, modification dates, and size.
#
...
#    Identical means: (a) sizes are identical AND (b) timestamps are within a user-defined time tolerance.
#    An optional hash check (--hash-check) can be enabled for stricter identity verification,
#    in which case files are identical ONLY if their content hashes match (along with size).
#  6. Files existing in both source and target, which are DIFFERENT, and source is newer.
#  7. Files existing in both source and target, which are DIFFERENT, and target is newer.
...
# Users can specify patterns to exclude certain files or folders from the comparison,
# either directly via command-line arguments or by providing a file containing the patterns.
#
# Note on timestamps (modification time):
# Sometimes, when manipulating files, the modification timestamp may be rounded to the nearest second.
# Using --time-tolerance 1 should indicate that such files are to be treated as identical.
#
# Usage examples:
...
REPORT_IDENTICAL = "Files in Source AND Target that are IDENTICAL (same size AND timestamps within tolerance, NO HASH-CHECK)"
...
def compile_exclusion_patterns(patterns_list):
    ...
    root_specific_regex = re.compile(r'^([^/]+)/(.*)$')
...
def is_excluded(relative_path, is_dir_item, current_scan_folder_basename=None):
    ...
    if DEBUG_EXCLUDE_ENABLED and DEBUG_TARGET_PATTERNS:
        if any(target_str in normalized_item_path for target_str in DEBUG_TARGET_PATTERNS):
...
def _scan_folder(root_path, comparison_results_dict, is_source_scan_flag, ignore_case):
    ...
    print(f"{'Scanning folder'.ljust(29)}: {root_path} ...")
    ...
    #print(f"Finished scanning folder: '{root_path}'\n")
    print(f"{'Finished scanning folder'.ljust(29)}: {root_path}\n")
    ...
    for relative_path in all_relative_paths:
        ...
        if source_item and not target_item:
            if source_item.is_dir:
                comparison_results_dict[REPORT_ONLY_IN_SOURCE].append(relative_path + os.sep)
            else:
                comparison_results_dict[REPORT_ONLY_IN_SOURCE].append(relative_path)
            continue
        elif target_item and not source_item:
            if target_item.is_dir:
                comparison_results_dict[REPORT_ONLY_IN_TARGET].append(relative_path + os.sep)
            else:
                comparison_results_dict[REPORT_ONLY_IN_TARGET].append(relative_path)
            continue
        ...
        if source_item.is_dir and target_item.is_dir:
            comparison_results_dict[REPORT_IDENTICAL].append(relative_path + os.sep)
            continue
        ...
        try:
            ...
            if source_item.size != target_item.size:
                ...
                if source_item.mtime_ns > target_item.mtime_ns:
                    comparison_results_dict[REPORT_DIFFERENT_CONTENT_SOURCE_NEWER].append(relative_path)
                    total_size_target_older_cat6 += target_item.size
                else:
                    comparison_results_dict[REPORT_DIFFERENT_CONTENT_TARGET_NEWER].append(relative_path)
            ...
            if abs(mtime1_s - mtime2_s) <= time_tolerance:
                ...
                if hash_check:
                    ...
                    if hash1 is None or hash2 is None:
                        ...
                        comparison_results_dict[REPORT_IDENTICAL].append(relative_path)
                else:
...
def main():
    ...
    if args.exclude_file:
        try:
            if os.path.exists(args.exclude_file):
                patterns_read_from_file_temp = [] # Utiliser une liste temporaire pour compter les motifs du fichier
                ...
                num_patterns_from_exclude_file = len(patterns_read_from_file_temp) # Stocker le nombre de motifs du fichier
    ...
    max_label_len = len("Exclusion patterns loaded from file") + 1

    print("\n" + "="*80)
    print("--------------------------- Folder Comparison Report ---------------------------")
    print("="*80)

    #print(f"\n{'Date'.ljust(39)}: {local_date_str}")
    #print(f"{'folder_comparator.py'.ljust(39)}: version {SCRIPT_VERSION} ({SCRIPT_DATE})")
    #print(f"{'Source'.ljust(39)}: {source_folder_cleaned}")
    #print(f"{'Target'.ljust(39)}: {target_folder_cleaned}")
    #print(f"{'Time Tolerance for IDENTICAL Files'.ljust(39)}: {args.time_tolerance} seconds")
    ...
    if args.hash_check:
        description_in_parentheses = "\n(Files considered IDENTICAL based on identical size and timestamp within tolerance, AND identical hash.)"
    else:
        description_in_parentheses = "\n(Files considered IDENTICAL based on identical size and timestamp within tolerance.)"
    ...
    for i, (category_key, category_title) in enumerate(report_categories_fixed_order):
        ...
        if category_key == REPORT_ONLY_IN_SOURCE: #
            ...
            for item_path in items:
                # Need to determine if it's a file or directory and get size from original scanned items
                original_item = source_items_scanned.get(item_path.rstrip(os.sep))
                if original_item:
                    if original_item.is_dir:
                        current_dirs += 1
                    else:
                        current_files += 1
                        if original_item.size is not None:
                            current_size += original_item.size
                else: # Fallback if item somehow not in scanned dict (e.g., if it was a broken symlink that also was only in source)
                    # Attempt to get info from filesystem if it wasn't in the scanned_items (e.g., if it was a broken symlink initially)
                    full_path_check = os.path.join(source_folder_cleaned, item_path.rstrip(os.sep))
            ...
            #print(f" "*4+f"Total: {total_source_only_dirs} directories, {total_source_only_files} files, size {format_size_human_readable(total_source_only_size)}") #
            ...
            current_size = 0
            for item_path in items:
                original_item = target_items_scanned.get(item_path.rstrip(os.sep))
                if original_item:
                    if original_item.is_dir:
                        current_dirs += 1
                    else:
                    ...
                    full_path_check = os.path.join(target_folder_cleaned, item_path.rstrip(os.sep))
                    if os.path.isdir(full_path_check):
                        current_dirs += 1
                    elif os.path.isfile(full_path_check):
                        current_files += 1
            ...
            #print(f" "*4+f"Total: {total_target_only_dirs} directories, {total_target_only_files} files, size {format_size_human_readable(total_target_only_size)}") #
            ...
            current_size = 0
            for item_path in items:
                original_item = source_items_scanned.get(item_path) # Identical means it's in both
                if original_item and not original_item.is_dir: # Only count files for size
                    current_files += 1
                    if original_item.size is not None:
                        current_size += original_item.size
            ...
            for item_path in items:
                ...
                if original_item and not original_item.is_dir:
                    current_files += 1
                    if original_item.size is not None:
                        current_size += original_item.size
            total_diff_source_newer_files = current_files
            total_diff_source_newer_size = current_size
            ...
            total_diff_target_newer_size = current_size
    ...
    print("Notes on categories 6. and 7.")
    net_change6 = format_size_human_readable(total_diff_source_newer_size-total_size_target_older_cat6)
    net_change7 = format_size_human_readable(total_diff_target_newer_size-total_size_source_older_cat7)
    l = max(len(net_change6),len(net_change7))
    print(f"6. {net_change6.rjust(l)}"+" = Net change in size of target_dir if the old files\n"+" "*(l+6)+f"were replaced by the new ones from source_dir")
    print(f"7. {net_change7.rjust(l)}"+" = Net change in size of source_dir if the old files\n"+" "*(l+6)+f"were replaced by the new ones from target_dir")
